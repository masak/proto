...
use Mojo::Util            qw/spurt  b64_encode/;
use Mojolicious;
...
sub spritify {
    ...
    my $asset_dir = $self->_gather_pics( @_ );
...
sub _gather_pics {
    ...
    find sub {
        return unless -f and /\.(png|gif|jpg|jpeg)$/;
...
use as sprites. Will croak if no paths are given or it has trouble
